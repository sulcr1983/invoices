...
@stats_bp.route('/api/stats/invoice-counts', methods=['GET'])
def get_invoice_counts():
    ...
    try:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM records WHERE verify_status = 'unverified'")
            verify_pending = cursor.fetchone()[0]
            cursor.execute("SELECT COUNT(*) FROM records WHERE deduction_status = 'unverified' AND invoice_type LIKE '%专%'")
            certify_pending = cursor.fetchone()[0]
            cursor.execute("SELECT COUNT(DISTINCT invoice_num) FROM records WHERE risk_flags IS NOT NULL AND risk_flags != ''")
            risk_count = cursor.fetchone()[0]
        finally:
            conn.close()
        return {
            ...
            'data': {
                ...
                'certify_pending': certify_pending,
                'risk_count': risk_count
            }
        }
    except Exception as e:
